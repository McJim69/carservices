	...
	if($trid!=null){
		...
		$insert = $pdo->prepare("INSERT INTO trans_details (
			...
			if($insert==TRUE){
				...
				echo"<script>location.href='transactions_details.php?transactions=$trid';</script>";
				...
				<div class='col'>
					<div class='row'>
						<div class='col-lg-12 form-group mt-3'>
							<div class='card'>
								<div class='card-body'>
									<h2 class="text-secondary">ADD PARTS TO JOB ORDER # <?php echo $tids;?></h2>	
									<button type="button" name="addTrans" class="btn btn-outline-success btn_addTrans" required>+Add</button> &nbsp;
									...
									<div class="box-footer text-center" style="margin:5px 10px 20px 10px">	
										<input style="margin:10px;width:100px" type="submit" onclick="jump('admin_transactions_add.php')" value="Reset" class="btn btn-outline-primary">
										<a href="javascript:history.back()" style="width:100px" class="btn btn-outline-warning">Cancel</a>
										<input style="margin:10px;width:100px" type="submit" name="transSave" value="Submit" class="btn btn-outline-success">
									</div>			
								</div>			
							</div>			
						</div>			
...
<script>
    $(document).ready(function(){
		$(document).on('click','.btn_addTrans', function(){
			var html='';
			html+='<tr class="addtr">';
			html+='<td style="display:none"><input  class="productname"  name="productname[]" type="hidden" readonly></td>';
